...
class BaseController(ABC):
    ...
    def update(
        ...
        if sensor_frame is None:
            # Always read the true sensor values first so raw_* telemetry
            # (used by metrics.py for error scoring) stays correct even
            # when a sensor is excluded from the control law below.
            raw_gyro_z = float(car.getGyro('z'))
            raw_euler_yaw = float(car.getEuler('yaw'))
            raw_accel_y = float(car.getAccel('y'))
            gyro_z = raw_gyro_z if self.use_gyro else 0.0
            euler_yaw = raw_euler_yaw if self.use_euler else 0.0
            accel_for_trim = raw_accel_y if (self.use_accel or self.lateral_accel_trim_enabled) else 0.0
